...
from panovggt.utils.geometry import closed_form_inverse_se3
...
def unproject_pano_depth_to_camera_coords(depth: torch.Tensor, shape: tuple) -> torch.Tensor:
    """
    Converts a panoramic depth map to 3D camera coordinates.

    Args:
        depth (torch.Tensor): Depth tensor of shape (1, H, W).
        shape (tuple): The resolution of the panoramic image (H, W).

    Returns:
        torch.Tensor: A tensor of 3D points in camera coordinates, shape (3, H, W).
    """
    h, w = shape
    device = depth.device
    ...
    # Convert to Cartesian directions (OpenCV convention: Z-forward, Y-down, X-right)
    ...
    camera_coords = directions * depth.permute(1, 2, 0) # Reshape depth to (H, W, 1) for broadcasting

    # format (H, W, 3)
    return camera_coords


def camera_coords_to_world_coords(cam_coords: torch.Tensor, R_w2c: np.ndarray, t_w2c: np.ndarray) -> torch.Tensor:
    """
    Transforms 3D points from camera coordinates to world coordinates.

    Args:
        cam_coords (torch.Tensor): 3D points in camera frame, shape (H, W, 3).
        R_w2c (np.ndarray): 3x3 rotation matrix (world-to-camera).
        t_w2c (np.ndarray): 3x1 translation vector (world-to-camera).

    Returns:
        torch.Tensor: 3D points in world frame, shape (H, W, 3).
    """
    H, W = cam_coords.shape[0], cam_coords.shape[1]
    cam_coords_flat = cam_coords.reshape(-1, 3)
    cam_coords_flat_np = cam_coords_flat.cpu().numpy()
    ...
    world_coords_flat = (R_c2w @ cam_coords_flat_np.T).T + t_c2w.reshape(1, 3)
    world_coords = world_coords_flat.reshape(H, W, 3)

    return torch.from_numpy(world_coords).float().to(cam_coords.device)


def transform_pano_track_points(track: torch.Tensor, rot_matrix: torch.Tensor, shape: tuple) -> torch.Tensor:
    """
    Transforms 2D equirectangular track points by a 3D rotation matrix.

    Args:
        track (torch.Tensor): Tensor of 2D points (N, 2) in (u, v) pixel coordinates.
        rot_matrix (torch.Tensor): 3x3 rotation matrix to apply (e.g., yaw_R_inv).
        shape (tuple): The image shape (H, W).

    Returns:
        torch.Tensor: The transformed 2D points (N, 2).
    """
    if track.shape[0] == 0:
        return track
    ...
    y = -torch.sin(phi)
    z = torch.cos(phi) * torch.cos(theta)
    d = torch.stack([x, y, z], dim=1).to(rot_matrix.device)

    # 2. Apply 3D rotation
    d_new = d @ rot_matrix.T

    # 3. Re-project new 3D direction vectors back to 2D
    x_new, y_new, z_new = d_new[:, 0], d_new[:, 1], d_new[:, 2]
    phi_new = torch.asin(-y_new.clamp(-1.0, 1.0))
    theta_new = torch.atan2(x_new, z_new)
    u_norm_new = (theta_new / (2 * math.pi)) + 0.5
