...
class DynamicTorchDataset(ABC):
    ...
    def get_loader(self, epoch):
        ...
        self.batch_sampler.set_epoch(epoch)  # batch_samplerepoch
...
class DynamicBatchSampler(Sampler):
    ...
    def __init__(
        ...
        self.np_rng = np.random.RandomState()  # numpy
    ...
    def set_epoch(self, epoch):
        ...
        # Keep per-epoch randomness deterministic across all ranks.
        common_seed = self.seed + epoch * 100
        self.rng.seed(common_seed)
        self.np_rng.seed(common_seed)
            ...
            try:
                ...
                for _ in range(batch_size):
                    try:
                        ...
                        print("End of sampler's iterator")
                        break  # No more samples

                if not current_batch:
                    break  # No more data to yield

                # Drop undersized tail batch if needed.
                actual_batch_size = len(current_batch)
                if actual_batch_size < drop_threshold and actual_batch_size != batch_size:
                    print(f"Dropping last batch with only {actual_batch_size} batches (min required: {drop_threshold})")
                    break  # drop small final batch
